// src/controllers/usuarioAutorizado.controller.js
import { UsuarioAutorizadoService } from "../services/usuarioAutorizado.service.js";
import { UserRepository } from "../repositories/user.repository.js";
import { usuarioAutorizadoResponseDTO } from "../dto/usuarioAutorizado.dto.js";
...
export const UsuarioAutorizadoController = {
  async crear(req, res) {
    try {
      ...
      return res.status(201).json({
        message: "✅ Usuario autorizado creado correctamente",
        user: usuarioAutorizadoResponseDTO(nuevo),
      });
    } catch (err) {
  ...
  async listar(_req, res) {
    try {
      ...
      return res.json(usuarios.map(usuarioAutorizadoResponseDTO));
    } catch (err) {
  ...
  async importar(req, res) {
    try {
      ...
      if (!Array.isArray(usuarios) || usuarios.length === 0) {
        return res
          .status(400)
          .json({ message: "No hay usuarios para importar" });
      }
      const resultados = await UsuarioAutorizadoService.importarUsuarios(
        usuarios
      );
  ...
  async validarCuitRegistro(req, res) {
    try {
      ...
      if (!empresa)
        return res.status(404).json({ message: "CUIT no autorizado" });

      if (empresa.password && empresa.password !== "") {
        return res.status(400).json({
          message: "Este CUIT ya tiene una cuenta registrada",
        });
      }
      ...
      return res
        .status(500)
        .json({ message: "Error interno al validar CUIT" });
    }
  },
};
